...
/** A short final month can consist entirely of days from the previous
 * month's connected week. That week (and its daily children) is already
 * generated by its owner; an empty local weekly cadence is valid here.
 */
function hasSharedWeek(cal: CalendarData, scope: PeriodRef, kind: RecipePeriod): boolean {
  return kind === "week" && scope.kind === "month" && cal.weeks.some((w) =>
    w.ownerMonthKey !== scope.key && w.days.some((d) => d.inRange && d.iso.slice(0, 7) === scope.key),
  );
}
